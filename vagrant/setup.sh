...
if ! grep -q "books.local.com" /etc/hosts; then
  echo "append to host file"
  echo "" >> /etc/hosts
  echo "127.0.0.1 books.local.com" >> /etc/hosts
fi
...
cp /home/vagrant/www/books/vagrant/books.conf /etc/apache2/sites-available/books.conf

rm -f /etc/apache2/sites-enabled/books.conf

ln -s /etc/apache2/sites-available/books.conf /etc/apache2/sites-enabled/books.conf


mysql -uroot -ppassword < /home/vagrant/www/books/vagrant/create.sql
